module Zohoho
  ...
  class Crm
    ...
    def initialize(auth_token, sandbox = false)
    ...
    def add_object(z_object, params = {})
      data = { xmlData: z_object.xml_data, newFormat: 1, wfTrigger: 'true' }
    ...
    def update_object(id, z_object, params = {})
      data = { id: id, xmlData: z_object.xml_data, newFormat: 1,
               wfTrigger: 'true' }
      data.merge!(params) if params.instance_of? Hash
    ...
    def add_contact(name)
      first_name, last_name = parse_name(name)
      xml_data = parse_data(
        { 'First Name' => first_name, 'Last Name' => last_name }, 'Contacts'
      )
      record = @conn.call(
        'Contacts', 'insertRecords', { xmlData: xml_data, newFormat: 1 }, :post
      )
      record['Id']
    end
    ...
    def post_note(entity_id, note_title, note_content)
      xml_data = parse_data({
                            ...
                            'Notes')
      record = @conn.call(
        'Notes', 'insertRecords', { xmlData: xml_data, newFormat: 1 }, :post
      )
      record['Id']
    end
