module Zohoho
  ...
  class Connection
    ...
    def zoho_uri
      "https://#{@service_name.downcase}"\
      "#{'sandbox' if @sandbox}.zoho.com/#{@service_name.downcase}/private/json"
    end
    ...
    def call(entry, api_method, query = {}, http_method = :get)
      ...
      when :post
        res = self.class.post(url, body: query).parsed_response
        res = res.to_json if res.is_a?(HTTParty::Response) || res.is_a?(Hash)
        raw = JSON.parse(res)
    ...
    def parse_error(response, query)
      error =
        "Zoho returned error code #{response['error']['code']}: "\
